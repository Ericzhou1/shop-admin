  ...
  <div>
    ...
    <div class="app-container">
        ...
        <el-col :span="24" class="flex">
          <el-form :inline="true" :model="formInline">
            ...
            <el-form-item style="width: 150px">
              ...
              >
                <el-option
                  v-for="item in orderStatusOptions.filter(
                    (item:any) => item.value !== 'WAIT_SUBMIT'
                  )"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
      ...
      >
        ...
        <el-table-column prop="companyName" label="商户名称" min-width="150" />
        ...
        >
          <template #default="scope">
            <div class="operation">
              ...
              >
                <template #reference>
                  <el-icon color="#F56C6C" size="20px"><Delete /></el-icon>
                </template>
              </el-popconfirm>
              <el-tooltip
                v-if="scope.row.orderStatus === 'WAIT_SHIP'"
                ...
                placement="top"
              >
                <el-icon @click="sendBox(scope.row)" size="20px">
                  <MessageBox />
                </el-icon>
              </el-tooltip>
            </div>
...
import { sellOrderList, orderStatusList } from '@/api/reportManager'
import DateUtil from '@/utils/getDate'
import { orderStatusOptions, orderTypeOptions } from '../pushOrder/optionsInfo'
...
const cancelOrder = (row: any) => {
  supplierSellOrderAgree({ orderId: row.orderId, agree: false })
    .then(() => {
